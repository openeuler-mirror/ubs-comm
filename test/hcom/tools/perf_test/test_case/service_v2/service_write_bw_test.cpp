...
constexpr uint16_t OP_SERVICE_WRITE_BW = 206;

int ServiceWriteBwTest::DoPostWrite()
{
...
int ServiceWriteBwTest::RequestReceived(const ock::hcom::UBSHcomServiceContext &ctx)
{
    int result = 0;
    if (mCfg.GetIsServer()) {
        // server
        UBSHcomRequest req(&mPostMrInfo, sizeof(mPostMrInfo), OP_SERVICE_WRITE_BW);
...
bool ServiceWriteBwTest::Initialize()
{
    sem_init(&mSem, 0, 0);
...
bool ServiceWriteBwTest::RegMemory()
{
...
bool ServiceWriteBwTest::ExchangeAddress()
{
    if (mCh == nullptr) {
    ...
    UBSHcomRequest req((void *)(const_cast<char *>(value.c_str())), value.length(), OP_SERVICE_WRITE_BW);
...
void ServiceWriteBwTest::UnInitialize()
{
    if (mCh != nullptr) {
...
bool ServiceWriteBwTest::Connect()
{
...
bool ServiceWriteBwTest::RunTest(PerfTestContext *ctx)
{
