 ...
 */
#ifndef UBS_COMM_GOLDEN_CRC_H
#define UBS_COMM_GOLDEN_CRC_H
...
class CRC {
public:
    static uint32_t Crc32(const uint8_t *buffer, uint32_t len);
};

inline uint32_t CRC::Crc32(const uint8_t *buffer, uint32_t length)
{
    static uint32_t CRC_TABLE[256L] = {
    ...
    for (uint32_t i = 0; i < length; i++) {
        crc = ((crc >> 8) ^ CRC_TABLE[((*p) ^ crc) & 0xFF]);
        p++;
    }
