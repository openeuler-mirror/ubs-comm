    ...
    void SetLocalTrxShm(Shm &input)
    {
        ...
        if (ret != 0) {
            RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                "Set local trx info copy local shared memory name failed, ret %d.\n",
                ret);
        }
    }
    ...
    int DoConnect()
    {
        ...
        if (SendSocketData(m_fd, &local_msg.protocol_negotiation, sizeof(uint64_t), NEGOTIATE_TIMEOUT_MS) != sizeof(uint64_t)) {
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET,
                "Add shmem connect failed to send protocol negotiation, fd: %d.\n",
                m_fd);
            return -1;
        }
        ...
        if (ret != 0) {
            RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                "Add shmem connect map local shared memory name %p failed, ret %d.\n",
                m_localTrxShm.name, ret);
            return -1;
        }
        ...
        if (ret != 0) {
            RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                "Add shmem connect copy local shared memory name failed, ret %d.\n",
                ret);
            shm->Unmap(&m_localTrxShm);
            shm->Free(&m_localTrxShm);
            return -1;
        ...
        if (SendSocketData(m_fd, &local_msg, sizeof(ExchangeData), NEGOTIATE_TIMEOUT_MS) != sizeof(ExchangeData)) {
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET,
                "Add shmem connect failed to send exchange data, fd: %d.\n",
                m_fd);
            shm->Unmap(&m_localTrxShm);
            shm->Free(&m_localTrxShm);
            return -1;
        ...
        if (RecvSocketData(m_fd, &remote_msg, sizeof(ExchangeData), NEGOTIATE_TIMEOUT_MS) != sizeof(ExchangeData)) {
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET,
                "Add shmem connect failed to recv remote exchange data, fd: %d.\n",
                m_fd);
            shm->Unmap(&m_localTrxShm);
            shm->Free(&m_localTrxShm);
            return -1;
        ...
        if (ret != 0) {
            RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                "Add shmem connect copy remote shared memory name failed, ret %d.\n",
                ret);
            shm->Unmap(&m_localTrxShm);
            shm->Free(&m_localTrxShm);
            return -1;
        ...
        if (SendSocketData(
            ...
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET, "Failed to send sync done message, fd: %d\n", m_fd);
            shm->Unmap(&m_localTrxShm);
            shm->Unmap(&m_remoteTrxShm);
            shm->Free(&m_localTrxShm);
            return -1;
        ...
        if (RecvSocketData(
            ...
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET, "Failed to receive sync done message, fd: %d\n", m_fd);
            shm->Unmap(&m_localTrxShm);
            shm->Unmap(&m_remoteTrxShm);
            shm->Free(&m_localTrxShm);
            return -1;
        ...
        switch (cmd) {
            case F_SETFL: // Set file status flags (O_NONBLOCK, etc.)
                ...
                if (ret == 0) {
                    ...
                    RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                        "fcntl set fd %d failed, ret is %d, errno %d\n",
                        m_fd, ret, errno);
                }
                break;
        ...
        switch (cmd) {
            case F_SETFL:
                ...
                if (ret == 0) {
                    ...
                    RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                        "fcntl64 set fd %d failed, ret is %d, errno %d\n",
                        m_fd, ret, errno);
                }
                break;
            case F_GETOWN: // Get process ID or process group ID receiving SIGIO/SIGURG signals
    ...
    ALWAYS_INLINE int Ioctl(int fd, unsigned long request, unsigned long int arg)
    {
        ...
        if (request == FIONBIO) {
            ...
            if (ret == 0) {
                ...
                RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
                    "ioctl set fd %d FIONBIO failed, ret is %d, errno %d\n",
                    m_fd, ret, errno);
            }
        } else {
...
private:
    ...
    int DoAccept(int new_fd)
    {
        ...
        if (RecvSocketData(new_fd, &remote_msg, sizeof(ExchangeData), CONTROL_PLANE_TIMEOUT_MS) !=
            sizeof(ExchangeData)) {
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET,
                "Accept shmem connect failed to recv remote exchange data, fd: %d.\n",
                new_fd);
        ...
        if (SendSocketData(new_fd, &local_msg, sizeof(ExchangeData), NEGOTIATE_TIMEOUT_MS) != sizeof(ExchangeData)) {
            RPC_ADPT_VLOG_ERR(ubsocket::NATIVE_SOCKET,
                "Accept shmem connect failed to send exchange data, fd: %d.\n",
                new_fd);
            shm->Unmap(&localTrxShm);
            shm->Unmap(&remoteTrxShm);
            shm->Free(&localTrxShm);
