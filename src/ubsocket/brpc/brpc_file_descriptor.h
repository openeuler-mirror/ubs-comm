...
public:
    ...
    static bool IsUBConnection(const int &fd)
    {
        UBHandshakeMode handshake_mode = Context::GetContext()->GetUbHandshakeMode();
        if (handshake_mode == UBHandshakeMode::TFO) {
            tcp_info info{};
        ...
        RPC_ADPT_VLOG_WARN("Unsupported handshake mode: 0x%x\n", static_cast<unsigned int>(handshake_mode));
        return false;
    }
    ...
    ALWAYS_INLINE int Accept(struct sockaddr *address, socklen_t *address_len)
    {
        ...
        if (fd >= 0) {
            // 前置判断：1、基于TFO握手模式：非TFO连接，作为普通TCP处理；2、基于ub_sock_opt握手模式：协议选项判断，不通过则作为普通TCP处理
            if (!IsUBConnection(fd)) {
                return fd;
            }
    ...
    int DoConnect(void)
    {
        ...
        // 客户端doroute后，明确当前topo类型，将其发给服务端
        if (SendSocketData(
            ...
            switch (state) {
                ...
                case UBHandshakeState::kRETRY:
                    ...
                    if (mTopoType == UMQ_TOPO_TYPE_CLOS) {
                        checkOtherRet = CLOSCheckOtherRoute(otherConnRoute, peerBondingEid, otherBackConnRoute);
                    } else {
                        checkOtherRet = CheckOtherRoute(otherConnRoute, peerBondingEid, connRoute);
                    }
                    if (checkOtherRet != 0) {
                        RPC_ADPT_VLOG_ERR(ubsocket::UBSocket,
    ...
    ALWAYS_INLINE int Connect(const struct sockaddr *address, socklen_t address_len)
    {
        ...
        if (m_tx_use_tcp || m_rx_use_tcp || !IsUBConnection(m_fd)) {
            return ret;
        }
...
private:
        ...
        std::vector<umq_route_t>& mainRoutes, std::vector<umq_route_t>& backRoutes)
    {
        ...
        // 本端的套接字，对端的套接字是mServerSocketIdForAffinity
        Context *context = Context::GetContext();
        int processSocketId = context->GetProcessSocketId();
        // 本端与对端的套接字数组
        std::vector<uint32_t> socketIds = context->GetAllSocketId();
        ...
        // 套接字在套接字数组的位置，也就是芯片在芯片数组的位置，从而算出本端与对端用何芯片
