 ...
 */
#ifndef UBS_COMM_UMQ_BUF_LIST_H
#define UBS_COMM_UMQ_BUF_LIST_H
...
#define QBUF_LIST_INSERT_HEAD(head, element)                 \
    do {                                                     \
        QBUF_LIST_NEXT((element)) = QBUF_LIST_FIRST((head)); \
        QBUF_LIST_FIRST((head)) = (element);                 \
    } while (0)

#define QBUF_LIST_INSERT_AFTER(element1, element2)           \
    do {                                                     \
...
#define QBUF_LIST_FOR_EACH(element, head) \
    for ((element) = QBUF_LIST_FIRST((head)); (element); (element) = QBUF_LIST_NEXT(element))

#define QBUF_LIST_FOR_EACH_SAFE(element, head, next) \
    for ((element) = QBUF_LIST_FIRST((head)); (element) && ((next) = QBUF_LIST_NEXT(element), 1); (element) = (next))

#define QBUF_LIST_REMOVE_HEAD(head)                                        \
    do {                                                                   \
        QBUF_LIST_FIRST((head)) = QBUF_LIST_NEXT(QBUF_LIST_FIRST((head))); \
    } while (0)

#define QBUF_LIST_REMOVE_AFTER(element)                                    \
    do {                                                                   \
        QBUF_LIST_NEXT(element) = QBUF_LIST_NEXT(QBUF_LIST_NEXT(element)); \
    } while (0)

#define QBUF_LIST_REMOVE(head, element, type)                \
    do {                                                     \
