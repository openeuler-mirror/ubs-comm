...
Result UmqConnHelper::NewBaseUmqCreateOptions(umq_create_option_t &umq_create_option, ub_trans_mode trans_mode)
{
    ...
    if (GetTpInfo(umq_create_option.tp_mode, umq_create_option.tp_type, trans_mode) != UBS_OK) {
        return UBS_ERROR;
    }

    if (GlobalSetting::UBS_ENABLE_SHARE_JFR && UmqSetting::UMQ_TP_TYPE == POOL) {
...
{
    return UmqBackend::InitShareJfrMonitering(main_umq_handle);
}
