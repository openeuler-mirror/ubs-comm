...
private:
    ...
    void CreateDirectoryExt(std::string &path)
    {
        if (path.empty()) {
            path = DEFAULT_DUMP_PATH_EXT;
        }

        constexpr mode_t DEFAULT_DIR_PERMISSION_EXT = 0750;
        std::string tmp_str = path;

        for (size_t i = 1; i < tmp_str.size(); ++i) {
            if (tmp_str[i] == '/') {
                tmp_str[i] = '\0';
                mkdir(tmp_str.c_str(), DEFAULT_DIR_PERMISSION_EXT);
                tmp_str[i] = '/';
            }
        }
    ...
    void WriteDumpDataExt(std::ostringstream &oss)
    {
        if (file_name_.empty()) {
