...
#ifdef __aarch64__
static inline uint32_t CalcCrc64Bit(const uint8_t **buffer, uint32_t len, uint32_t crc)
{
    auto tmpBuf = reinterpret_cast<const uint64_t *>(*buffer);
    auto crc64 = (uint64_t)crc;

    for (uint32_t index = 0; index < len; index++) {
        __asm__ volatile("crc32cx %w[c], %w[c], %x[v]" : [c] "+r"(crc64) : [v] "r"(*tmpBuf++));
    }

    *buffer = reinterpret_cast<const uint8_t *>(tmpBuf);
    return (uint32_t)crc64;
}

static inline uint32_t CalcCrc32Bit(const uint8_t **buffer, uint32_t len, uint32_t crc)
{
    auto tmpBuf = reinterpret_cast<const uint32_t *>(*buffer);
    for (uint32_t index = 0; index < len; index++) {
        __asm__ volatile("crc32cw %w[c], %w[c], %w[v]" : [c] "+r"(crc) : [v] "r"(*tmpBuf++));
    }
...
static inline uint32_t CalcCrc16Bit(const uint8_t **buffer, uint32_t len, uint32_t crc)
{
    auto tmpBuf = reinterpret_cast<const uint16_t *>(*buffer);
    for (uint32_t index = 0; index < len; index++) {
        __asm__ volatile("crc32ch %w[c], %w[c], %w[v]" : [c] "+r"(crc) : [v] "r"(*tmpBuf++));
    }
...
static inline uint32_t CalcCrc8Bit(const uint8_t **buffer, uint32_t len, uint32_t crc)
{
    auto tmpBuf = *buffer;

    for (uint32_t index = 0; index < len; index++) {
        __asm__ volatile("crc32cb %w[c], %w[c], %w[v]" : [c] "+r"(crc) : [v] "r"(*tmpBuf++));
    }
...
uint32_t NetCrc32C(const void *buffer, uint32_t length)
{
    uint32_t crc = 0xffffffff;
    uint32_t len = length;

    if (NN_UNLIKELY(buffer == nullptr || len == 0)) {
        return 0;
    }

    auto tmpBuf = reinterpret_cast<const uint8_t *>(buffer);

    // 首地址可能不对齐，考虑到 UBSHcomNetTransHeader 头部较小，计算时仅需 24 字节，所
    // 以采用 crc32c 4B 硬件指令而非 8B 以带来较好的 throughput、较低的 latency.
    constexpr uint32_t STEP = 4;
    const uint32_t toAlign = static_cast<uint32_t>(-(uintptr_t)tmpBuf & (STEP - 1));
    switch (toAlign) {
        case NN_NO3:
            __asm__ volatile("crc32cb %w[c], %w[c], %w[v]" : [c] "+r"(crc) : [v] "r"(*tmpBuf++));
            // fallthrough
    ...
    length -= toAlign;
    for (; length >= STEP; length -= STEP, tmpBuf += STEP) {
        __asm__ volatile("crc32cw %w[c], %w[c], %w[v]" : [c] "+r"(crc) : [v] "r"(*(uint32_t *)tmpBuf));
    }
...
#if defined(__x86_64__) /* for x86_64 */
static inline uint32_t CalcCrc64Bit(const uint8_t **buffer, uint32_t len, uint32_t crc)
{
    const uint8_t *tmpBuf = *buffer;
    auto crc64 = (uint64_t)crc;

    for (uint32_t index = 0; index < len; index++) {
        __asm__ volatile("crc32q\t"
                         "(%1), %0"
                         : "+r"(crc64)
                         : "r"(tmpBuf), "m"(*tmpBuf));
...
uint32_t NetCrc32C(const void *buffer, uint32_t length)
{
    ...
    if (NN_UNLIKELY(len == 0 || buffer == nullptr)) {
        return 0;
    }

    auto tmpBuf = reinterpret_cast<const uint8_t *>(buffer);

    /* The performance of the crc32 command is better than that of the __mm_crc32/__built-in _ia32_crc32 command. */
#if defined(__x86_64__)
    crc = CalcCrc64Bit(&tmpBuf, len / sizeof(uint64_t), crc);
    len &= sizeof(uint64_t) - 1;
#endif
    crc = CalcCrc32Bit(&tmpBuf, len / sizeof(uint32_t), crc);
    len &= sizeof(uint32_t) - 1;

    crc = CalcCrc16Bit(&tmpBuf, len / sizeof(uint16_t), crc);
    len &= sizeof(uint16_t) - 1;

    crc = CalcCrc8Bit(&tmpBuf, len / sizeof(uint8_t), crc);

    return ~crc;
}
#endif

#else // USE_SOFTWARE_CRC
// CRC-32C Poly:1EDC6F41
static uint32_t gCrcTable[NN_NO256] = {
...
uint32_t NetCrc32C(const void *buffer, uint32_t length)
{
    auto p = reinterpret_cast<const unsigned char *>(buffer);
    uint32_t crc = 0xFFFFFFFF;
    for (uint32_t i = 0; i < length; i++) {
        crc = ((crc >> NN_NO8) ^ gCrcTable[((*p) ^ crc) & 0xFF]);
        p++;
    }
...
#endif /* __cplusplus */
Crc32Function NetCrc32::gCrc32Func = NetCrc32C;
} // namespace hcom
} // namespace ock
