...
inline const char *HcomTimerDiagVersion()
{
    ...
    static const std::string ver = []() {
        ...
        int month = 0;
        switch (d[0]) {
            case 'J': month = (d[1] == 'a') ? 1 : (d[2] == 'n' ? 6 : 7); break; /* Jan / Jun / Jul */
            case 'F': month = 2; break;                                        /* Feb */
            case 'M': month = (d[2] == 'r') ? 3 : 5; break;                    /* Mar / May */
            case 'A': month = (d[1] == 'p') ? 4 : 8; break;                    /* Apr / Aug */
        ...
        char buf[32] = {0};
        (void)snprintf_s(buf, sizeof(buf), sizeof(buf) - 1, "%04d%02d%02d-%c%c%c%c%c%c",
                       year, month, day, t[0], t[1], t[3], t[4], t[6], t[7]);
