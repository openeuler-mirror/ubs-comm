...
#define OCK_HCOM_NET_SHM_ASYNC_ENDPOINT_H
...
namespace ock {
namespace hcom {
class NetAsyncEndpointShm : public NetEndpointImpl {
public:
    NetAsyncEndpointShm(uint64_t id, ShmChannel *ch, ShmWorker *worker, NetDriverShmWithOOB *driver,
                        const UBSHcomNetWorkerIndex &workerIndex, ShmMRHandleMap &handleMap);
    ~NetAsyncEndpointShm() override;

    NResult SetEpOption(UBSHcomEpOptions &epOptions) override
    {
        NN_LOG_WARN("[SHM AsyncEp] Empty function for now");
        return NN_OK;
    }
    ...
    inline const std::string &PeerIpAndPort() override
    {
        if (NN_LIKELY(mShmCh != nullptr)) {
    ...
    {
        NN_LOG_WARN("Shm Invalid operation, wait completion is not supported by NetAsyncEndpoint");
    ...
    NResult SendFds(int fds[], uint32_t len) override
    {
        if (NN_UNLIKELY(len < NN_NO1 || len > NN_NO4)) {
            NN_LOG_ERROR("Shm Failed to send fds in shm async ep as length should more than 0 and less than 4.");
            return NN_PARAM_INVALID;
        }

        if (NN_UNLIKELY(!mState.Compare(NEP_ESTABLISHED))) {
            NN_LOG_ERROR("Shm Failed to send fds in shm async ep as endpoint "
                         << mId << " is not established, state is " << UBSHcomNEPStateToString(mState.Get()));
        ...
        for (uint32_t i = 0; i < len; i++) {
            innerFds[i] = fds[i];
            if (fds[i] <= 0) {
                NN_LOG_ERROR("Shm Failed to send fds in shm async ep, as invalid fds index:" << i);
        ...
        if (NN_UNLIKELY(::send(mShmCh->UdsFD(), &header, sizeof(ShmChKeeperMsgHeader), MSG_NOSIGNAL) <= 0)) {
            ...
            NN_LOG_ERROR("Shm Failed to send header info of exchange external fd to peer, error "
    ...
    {
        if (NN_UNLIKELY(len < NN_NO1 || len > NN_NO4)) {
            NN_LOG_ERROR("Shm Failed to receive fds in shm async ep as length should more than 0 and less than 4.");
            return NN_PARAM_INVALID;
        }

        if (NN_UNLIKELY(!mState.Compare(NEP_ESTABLISHED))) {
            NN_LOG_ERROR("Shm Failed to receive fds in shm async ep as endpoint "
                         << mId << " is not established, state is " << UBSHcomNEPStateToString(mState.Get()));
            return NN_EP_NOT_ESTABLISHED;
        }
    ...
    NResult GetRemoteUdsIdInfo(UBSHcomNetUdsIdInfo &idInfo) override
    {
        if (!mState.Compare(NEP_ESTABLISHED)) {
            NN_LOG_ERROR("[SHM AsyncEp] EP is not established");
            return NN_EP_NOT_ESTABLISHED;
        }

        if (!mDriver->mStartOobSvr) {
            NN_LOG_ERROR("[SHM AsyncEp] oob server is not start");
    ...
    {
        NN_LOG_WARN("Shm Invalid operation for shm, shm does not have ip and port");
        return false;
    }

private:
    bool inline NeedRetry(HResult &result)
    {
        ...
        if (result == SH_OP_CTX_FULL || result == SH_RETRY_FULL) {
            return true;
        }
...
#endif // OCK_HCOM_NET_SHM_ASYNC_ENDPOINT_H
