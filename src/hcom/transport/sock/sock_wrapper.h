...
#define TRACE_SEND_CTX_QUEUED(ctx)                                                                                     \
    do {                                                                                                               \
        ++mDbgSendCtxQueued;                                                                                           \
        NN_LOG_TRACE_INFO("[OPCTX-LEAK] sock " << mId << " queued send ctx " << static_cast<void *>(ctx) << " opType " \
                                               << static_cast<int>((ctx)->opType) << ", queued "                       \
                                               << mDbgSendCtxQueued);                                                  \
    } while (0)
...
#define TRACE_SEND_CTX_DRAINED(popReq)                                                                               \
    do {                                                                                                             \
        ...
        if (NN_UNLIKELY((mDbgSendCtxDrained & 0xFFFULL) == 0)) {                                                     \
            NN_LOG_WARN("[OPCTX-LEAK] sock "                                                                         \
                        << mId << " drained send ctx " << static_cast<void *>(popReq) << " opType "                  \
                        << static_cast<int>((popReq)->opType) << " WITHOUT returning it to the op ctx pool, queued " \
                        << mDbgSendCtxQueued << ", drained " << mDbgSendCtxDrained << ", send queue size "           \
                        << mSendQueue.Size());                                                                       \
        }                                                                                                            \
    } while (0)
#else
